...
public class SimpleUpgradePanel : MonoBehaviour
{
    ...
    private void EnsureNodes()
    {
        ...
        if (debugLayout)
        {
            ...
            sb.Append("[UpgLayout] Sorted defs: ");
            for (int i = 0; i < definitions.Count; i++)
            {
                var d = definitions[i];
                if (d == null) continue;
                sb.Append("(").Append(d.id).Append(", idx=").Append(d.index).Append(") ");
            }
    ...
    private GameObject CreateNode(UpgradeDefinition def, Vector2 anchoredPosition)
    {
        ...
        button.onClick.AddListener(() =>
        {
            ...
            if (bought)
            {
                ...
                try
                {
                    Debug.Log($"[UpgradeClick] Bought {def.id} -> level {UpgradeSystem.GetLevel(def)}");
                }
                catch { }
            }
            RefreshUI();
    ...
    private Vector2 GetAnchoredPosition(UpgradeDefinition def)
    {
        ...
        if (def.layoutDir == LayoutDirection8.None)
        {
            ...
            if (debugLayout)
                Debug.Log($"[UpgLayout] {def.id}: layout=None index={def.index} -> pos={p}");
            return p;
        }

        // Find anchor: first prerequisite node; fallback to center
        Vector2 anchorPos = Vector2.zero;
        if (def.prerequisites != null && def.prerequisites.Count > 0)
        {
            var pre = def.prerequisites[0].upgrade;
            if (pre != null && nodeByDef.TryGetValue(pre, out var anchorGo) && anchorGo != null)
            {
                var rt = anchorGo.GetComponent<RectTransform>();
                if (rt != null)
                    anchorPos = rt.anchoredPosition;
                if (debugLayout)
                    Debug.Log($"[UpgLayout] {def.id}: anchored to prereq {pre.id} at {anchorPos}");
            }
            else if (debugLayout)
            {
                string preId = pre != null ? pre.id : "<null>";
                Debug.Log($"[UpgLayout] {def.id}: prereq {preId} not spawned; fallback to center (0,0)");
            }
        }
    ...
    private void RedrawLines()
    {
        ...
        foreach (var kv in nodeByDef)
        {
            ...
            // Prefer prerequisite as anchor for line
            if (kv.Key != null && kv.Key.prerequisites != null && kv.Key.prerequisites.Count > 0)
            {
                var pre = kv.Key.prerequisites[0].upgrade;
                if (pre != null && nodeByDef.TryGetValue(pre, out var preGo) && preGo != null)
                {
                    var preRt = preGo.GetComponent<RectTransform>();
                    if (preRt != null)
                        a = preRt.anchoredPosition;
                }
            }
    ...
    private void ReanchorNodes()
    {
        ...
        for (int i = 0; i < definitions.Count; i++)
        {
            ...
            // Require a prerequisite anchor
            if (def.prerequisites == null || def.prerequisites.Count == 0) continue;
            var pre = def.prerequisites[0].upgrade;
            if (pre == null) continue;
            if (!nodeByDef.TryGetValue(pre, out var anchorGo) || anchorGo == null) continue;
